...
class Yolo:
    ...
    def process_outputs(self, outputs, image_size):
        ...
        for o, output in enumerate(boxes):
            for y in range(output.shape[0]):
                for x in range(output.shape[1]):
                    c = ((1 / (1 + np.exp(-output[y, x, :, 1])) + y)
                               / output.shape[0] * image_size[0])
                    cx = ((1 / (1 + np.exp(-output[y, x, :, 0])) + x)
                               / output.shape[1] * image_size[1])
                    prior_resizes = self.anchors[o].astype(float)
                    ...
                    output[y, x, :, 0] = cx - prior_resizes[:, 0]
                    output[y, x, :, 1] = c - prior_resizes[:, 1]
                    output[y, x, :, 2] = cx + prior_resizes[:, 0]
                    output[y, x, :, 3] = c + prior_resizes[:, 1]
        box_confidences = [1 / (1 + np.exp(-output[:, :, :, 4, np.newaxis]))
                           for output in outputs]
        box_class_ps = [1 / (1 + np.exp(-output[:, :, :, 5:]))
                           for output in outputs]
        return boxes, box_confidences, box_class_ps
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        ts = np.where(all_confidences < self.class_t)
        return (np.delete(all_boxes, ts, axis=0),
                np.delete(all_classes, ts),
                np.delete(all_confidences, ts))
